...
var mapstyleSatilite = "mapbox://styles/mapbox/satellite-v9"; //can change
...
document.getElementById("search").addEventListener("submit", function (e) {
  e.preventDefault();
  searchresult = null; //@ts-ignore

  searchresult = document.getElementById("nav-search").value;
  var target;
  console.log(searchresult);

  if (searchresult == "movies" || "theater") {
    target = isAtStart ? movies : centerMap;
    isAtStart = !isAtStart;
    map.flyTo({
      center: target,
      zoom: 17,
  ...
  } else if (searchresult == "park" || "hello") {
    target = isAtStart ? parkbyTOHS : centerMap;
    isAtStart = !isAtStart;
    map.flyTo({
      center: target,
      zoom: 17,
  ...
  } else if (searchresult == "library") {
    target = isAtStart ? library : centerMap;
    isAtStart = !isAtStart;
    map.flyTo({
      center: target,
      zoom: 17,
